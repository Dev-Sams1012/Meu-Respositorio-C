...
*       *\n espaço antes=0; espaço meio=7
 *     *\n  espaço antes=1; espaço meio=5
  *   *\n   espaço antes=2; espaço meio=3
   * *\n    espaço antes=3; espaço meio=1
    *\n     espaço antes=4;
...
void __letraVaux(int n, int espaco_antes, int espaco_meio){
	if(n == 1){
		print_space(espaco_antes);
		print_asterisco(1);
		pula_linha();
	}else{
		print_space(espaco_antes);
		print_asterisco(1);
		print_space(espaco_meio);
		print_asterisco(1);
		pula_linha();
		__letraVaux(n-1,espaco_antes+1,espaco_meio-2);
	}
}

void letraV(int n){
	__letraVaux(n,0,2*n-3);
	return;
}
...
void __bandeiraaux(int n,int espaco_meio){
	if(n == 1){
		print_asterisco(1);
		print_space(espaco_meio);
		print_asterisco(1);
		...
		__bandeiraaux(n-1,espaco_meio+2);
