...
/*int avalia_polenesa_reversa(lista_tokens * l2){
	...
	v = atoi(get_valor(l2, 0) ); 
	push(&p, v);

	v = atoi(get_valor(l2, 1) ); 
	push(&p, v);

	int op2 = pop(&p);
	int op1 = pop(&p);
	int res = calcula(get_categoria(l2, 2), op1 , op2);
...
/*int avalia_polenesa_reversa(lista_tokens * l2){
	...
	while(get_categoria(l2,i) == OP){
		int val = atoi(get_valor(l2, i));
		push(&p,val);
		i++;
	}
	...
	while(p.topo != 0){
		...
		printf("topo = %d, abaixo = %d\n",topo,abaixo);

		res = calcula(get_categoria(l2, i), topo , abaixo);
...
int avalia_polenesa_reversa(lista_tokens * l2){
	...
	while(i < l2->qtd_tokens){ // considerando o tamanho da Lista de Tokens recebida, para pegar todas informações.
		enum classe  tipo = get_categoria(l2, i); // varíavel do tipo enum classe, pois as operações feitas na pilha dependem se os elementos forem operandos ou operadores.

		if (tipo == OP){ // se for um numero, eu pego o valor inteiro deste número e acrescento na pilha.
			int val = atoi(get_valor(l2, i));
			push(&p,val);
		}
		
		else if(tipo == PLUS || tipo == SUB || tipo == MUL || tipo == DIV){ // mas se for um operador, eu faço a conta com os dois ultimos termos da pilha, e o resultado acrescento na pilha.
			int valor_1 = pop(&p);
			int valor_2 = pop(&p);

			int res = calcula(get_categoria(l2,i),valor_1,valor_2);
	...
	return pop(&p); // como as operações foram feitas corretamente, restará apenas um termo na pilha, que é o resultado da avaliação do token na notação polonesa reversa.
}
